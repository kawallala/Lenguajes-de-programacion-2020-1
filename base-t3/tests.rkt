...
;minimal get
(test (run-val '(send (object (field y 2) (method get-y () (get y))) get-y)) 2)

;get needs to be inside of an object
(test/exn (run-val '(+ 2 (get x))) "get used outside of an object")
...
;set need to be inside of an object
(test/exn (run-val '(set x 2)) "set used outside of an object")

;fields can not be named this
(test/exn (run-val '(object (field this 3))) "this is a reserved word")
...
;can't fiend fields outside myself
(test/exn (run-val '(local
               ([define x (object
                           (field z 3)
                           (method get () (get u)))]
...
;bonus reloaded with more than 1 argument
(test (run-val '(local
              [(define f (fun (x y)
                              (+ x y)))]
              (f 5 20)))
