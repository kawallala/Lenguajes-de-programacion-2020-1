...
(deftype Val
  (numV n)
  (closureV x b e)
  (exprV expr env cache)); cache : Box[#f o numV/closureV]
    ...
    [(app f e)
      ...
      (interp b
              (extend-env p
                          ...
                          (exprV e env (box #f));now we're being lazy! (empty cache)
...
(define (strict v)
  (match v
    ...
    [(exprV expr env cache)
     ...
     (if c
         (begin
           (printf "using cache: ~a~n" c)
           c)
         (let
           ([val (strict(interp expr env))])
           (printf "Forcing exprV to ~a~n" val)
...
(test (run '{with {x {+ 4 5}}
                  {with {y {+ x x}}
                        {with {x 2}
                              y}}})
      18)
;; dos semanticas posibles para postergar evaluacion:
; 1. (clase 21/04) "call-by-name": evalua proemsa cada vez que se necesita
; 2. (clase 23/04) "cal-by-need" (o "lazy evaluation"): evalua promesa a lo mas 1 vez

;Haskell: call-by-need
;Scala: call-by-val call-by-need call-by-name (con distintos keywords/notaciones)

;why call-by-name?
; para operadores de iteracion (for/while/do-while/etc
; while <c> do <b>
(run '{with {x {+ 1 7}}
       {with {y {+ x x}}
             {with {z {+ y y}}
